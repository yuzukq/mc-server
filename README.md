...
Dockerコンテナ上でMinecraftサーバーを立ち上げ、Cloudflare R2を使って複数のホスト間でワールドデータを共有するシステムです(現在お試しで改良しながら運用中)。

## できること
- 🐳 **Docker統合**: バッチファイルで起動・停止
- 🔒 **ロック機構**: R2上のロックファイルで同時起動を防止
- 🔄 **自動同期**: サーバー起動時にR2からワールドをダウンロード、停止時にアップロード
- 📺 **ホスト表示**: 現在のホストをゲーム内bossbarで全プレイヤーに表示
- 🔔 **参加通知**: Discord　Webhookを使ってサーバのステータス、参加人数を通知します
...
| サービス | 役割 | 実行タイミング |
|---------|------|--------------|
| `sync-init` | R2のLock取得・ワールドデータDL | サーバー起動前 |
| `server` | Minecraft サーバー (Forge 1.20.1) | sync-init 完了後 |
| `bossbar-manager` | ホスト名をゲーム内Bossbarに表示 | サーバー起動後 |
| `discord-notify` | サーバー・プレイヤーイベントをDiscordに通知 | サーバー起動後 |
| `sync-shutdown` | ワールドデータUL・Lock解放 | サーバー停止後 |
...
sequenceDiagram
    ...
    participant MC as MC Server

    Note over U,MC: 起動フロー
    U->>DC: start-server.sh
    DC->>S: sync-init 起動
    S->>R2: Lock確認
    S->>R2: Lock作成
    S->>R2: ワールドデータ Download
    S-->>DC: 完了
    DC->>MC: サーバー起動
    Note over MC: bossbar-manager, discord-notify も起動

    Note over U,MC: 停止フロー
    U->>DC: stop-server.sh
    DC->>MC: サーバー停止 (compose down)
    DC->>S: sync-shutdown 起動
    S->>R2: ワールドデータ Upload
    S->>R2: Lock解放
```
...
# .env.dev を編集: 開発用バケット名(本番環境とは別に新たに作成してください)を設定
...
```
 ...
 ├── scripts/                 # スクリプト
 │   ├── lib/
 │   │   └── rcon_client.rb   # RCON通信クライアント(共通)
 │   ├── sync.rb              # R2同期スクリプト
 │   ├── bossbar.rb           # ホスト表示bossbar管理スクリプト
 │   └── discord_notify.rb    # Discord通知スクリプト
